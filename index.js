...
function parseInput(rplyToken, inputStr) {
	...
	let trigger = mainMsg[0].toString().toLowerCase(); //指定啟動詞在第一個詞&把大階強制轉成細階
        ...
        if (trigger.toLowerCase().match(/^\d+d\d+/) != null && trigger.toLowerCase().match(/\d$/) != null && trigger.toLowerCase().match(/[a-c]|[e-z]|[`-~!@#$%^&*()_=]/gi) == null) 
	{
...
varsu++;
	...
	dicemax = Math.max(...wodarray);	//判斷最大最小值
	dicemin = Math.min(...wodarray);

	if (Number(match[1]) == 1)
		returnStr += dicemax + '[' + wodarray.pop() + ']'; 
	else
	{
		returnStr += dicemax + '[';
		for (i = 0; i < Number(match[1]); i++)
